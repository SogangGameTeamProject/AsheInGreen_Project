...
namespace AshGreen.Buff
{
    ...
    public class OverflowingPower_Buff : BuffData
    {
        ...
        {
            player.AddAttackpowerRpc(0, buff.baseVal[0] + (buff.stackVal[0] * (buff.currentStacks-1)));// 공격력 증가
            player.AddCriticalRpc(buff.baseVal[1] + (buff.stackVal[1] * (buff.currentStacks - 1)));// 크리티컬 확률 증가
        }
        ...
        {
            player.AddAttackpowerRpc(0, -(buff.baseVal[0] + (buff.stackVal[0] * (buff.currentStacks - 1))));// 공격력 감소
            player.AddCriticalRpc(-(buff.baseVal[1] + (buff.stackVal[1] * (buff.currentStacks - 1))));// 크리티컬 확률 감소
        }
    }
