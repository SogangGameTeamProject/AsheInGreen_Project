...
namespace AshGreen.Buff
{
    ...
    public class Adrenaline_Buff : BuffData
    {
        ...
        {
            player.AddMovespeedRpc(0, baseVal[0] + (stackIncVal[0] * (buff.currentStacks-1)));// 이동 속도 증가
            player.AddJumpRpc(0, baseVal[0] + (stackIncVal[0] * (buff.currentStacks - 1)));// 점프력 증가
        }
        ...
        {
            player.AddMovespeedRpc(0, -(baseVal[0] + (stackIncVal[0] * (buff.currentStacks - 1))));// 이동 속도 감소
            player.AddJumpRpc(0, -(baseVal[0] + (stackIncVal[0] * (buff.currentStacks - 1))));// 점프력 감소
        }
    }
